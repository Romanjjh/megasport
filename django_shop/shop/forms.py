...
class RegisterUserForm(UserCreationForm):
    ...
    password1 = forms.CharField(label='Пароль', widget=forms.PasswordInput(attrs={'class': 'form-input'}), min_length=8,
                                error_messages={'min_length': 'Пароль має бути довжиною не менше 8 символів'})
    ...
    def clean_password2(self):
        ...
        if password1 and password2 and password1 != password2:
            raise forms.ValidationError('Паролі не сходяться')
        return password2

    def clean_password1(self):
        password1 = self.cleaned_data.get('password1')
        if len(password1) < 8:
            raise forms.ValidationError('Пароль має бути довжиною не менше 8 символів.')
        if password1.isdigit():
            raise forms.ValidationError('Пароль має містити хоча б один символ, окрім цифр.')
        return password1
...
class FeedbackForm(forms.Form):
    ...
    capatcha = CaptchaField(label='Введіть каптчу з зображення')
